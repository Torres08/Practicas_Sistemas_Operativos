...
#define longnombre 256

static void mimanejador(int senal)
{
    printf("Cliente %d: escribo en el FIFO aún no abierto para lectura\n", getpid());
    return;
}
...
int obtenerfifo(int dfifoe, int dfifos, int mipid)
{
    ...
    {
        perror("Cliente: error al escribir petición en fifo conocido del servidor");
        exit(EXIT_FAILURE);
    }
    ...
    printf("Cliente %d: leído pid %d en fifo de servidor. \n", mipid, pidproxy);

    sprintf(nombrefifoproxy, "./papelera/cliente_fifo_%d", pidproxy);
    mkfifo(nombrefifoproxy, 0666);
...
void producir(char c, int mififo, int mipid)
{
    ...
    if (signal(SIGPIPE, mimanejador) == SIG_ERR)
        perror("Cliente: Error configurando señal");
